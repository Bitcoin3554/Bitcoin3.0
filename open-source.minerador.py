...
def registrar_no_peer(peer_ip, meu_ip):
    url = f"http://{peer_ip}/register_node"
    try:
        resp = requests.post(url, json={'ip': meu_ip}, timeout=5)
        if resp.ok:
            print(f"Registrado no peer {peer_ip} com sucesso.")
    except Exception as e:
        print(f"Erro ao registrar no peer {peer_ip}: {e}")
...
def register_node():
    ...
    # Obtém IP real do cliente (considera proxy reverso via X-Forwarded-For)
    ip_cliente = request.headers.get('X-Forwarded-For', request.remote_addr)
    if ip_cliente and ',' in ip_cliente:
        ip_cliente = ip_cliente.split(',')[0].strip()

    # Valida dados recebidos e IP no JSON
    if not data or 'ip' not in data:
        return jsonify({"message": "IP inválido ou dados ausentes."}), 400

    ip_novo = data['ip']
    if ':' in ip_novo and not ip_novo.startswith('['):  # formata IPv6 para padrão
        ip_novo = f"[{ip_novo}]"

    # Se o peer já existir, atualiza o last_active e retorna mensagem
    if ip_novo in nodes:
        nodes[ip_novo]['last_active'] = time.time()
        return jsonify({"message": f"Peer {ip_novo} já registrado. Last active atualizado."}), 200

    # Caso seja novo peer, registra com timestamp atual
    nodes[ip_novo] = {
        'ip': ip_novo,
        'name': data.get('name', f"Nó {ip_novo}"),
        'last_active': time.time()
    }
    print(f"Novo peer registrado: {ip_novo}")
    return jsonify({"message": f"Peer {ip_novo} registrado com sucesso."}), 201
