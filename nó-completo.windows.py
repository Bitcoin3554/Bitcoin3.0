...
class KertOneCoreClient(QMainWindow):
    ...
    def enviar_transacao(self):
        ...
        """
        # 1) Verifica que a carteira já foi carregada
        if not self.wallet_data:
        ...
        # 3) Valida preenchimento
        if not recipient_addr or not amount_str or not fee_str:
            QMessageBox.warning(self, "Erro", "Todos os campos são obrigatórios.")
            return

        try:
            # 4) Converte e valida números
            amount = float(amount_str)
            fee    = float(fee_str)
            if amount <= 0 or fee < 0:
                raise ValueError("Valor ou taxa inválidos.")

            # 5) Gera um ID único para a transação
            transaction_id = str(uuid.uuid4())

            # 6) Formata strings com 8 casas decimais
            amount_fmt = f"{amount:.8f}"
            fee_fmt    = f"{fee:.8f}"

            # 7) Prepara dados para assinatura
            tx_data_for_signing = {
            ...
            # 9) Envia em thread para não travar a UI
