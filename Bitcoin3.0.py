...
class Blockchain:
    ...
    def calculate_hash(self, block):
        ...
        block_copy.pop("hash", None)  # Remove o campo hash antes de calcular
    ...
    def create_genesis_block(self):
        ...
        genesis_block = {
            ...
            "previous_hash": "0",
            "proof": 100,
            "timestamp": 1751892108.0,
            "transactions": []
        }
        # Note o uso do self.calculate_hash para chamar a função dentro da classe
        ...
        print("✅ Bloco gênese padronizado criado (cliente).")
    ...
    def create_new_block(self, previous_hash, proof, timestamp, miner, transactions=None):
        ...
        block['hash'] = self.hash_block(block)  # usar método estático da classe
        return block
...
def salvar_bloco_no_bd(block):
    conn = sqlite3.connect('blockchain.db')
    cursor = conn.cursor()
    try:
        # Exemplo: Insere ou atualiza um bloco
        cursor.execute('''
            INSERT OR REPLACE INTO blockchain (idx, block) VALUES (?, ?)
        ...
        for porta in portas:
            ...
            try:
                ...
                if 'chain' in data:
                    ...
                    # Salvar cada bloco recebido no banco antes de tentar substituir
                    for bloco in remote_blockchain_data:
                        salvar_bloco_no_bd(bloco)
