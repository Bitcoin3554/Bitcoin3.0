...
class Blockchain:
    ...
    def hash_block(self, block):
        # Copia o bloco e remove o campo 'hash' antes de calcular
            ...
            try:
                ...
                if 'chain' in data:
                    ...
                    if validar_chain(blockchain_data) and len(blockchain_data) > len(blockchain.chain):
                        ...
                        print("⚠️ Blockchain recebida é inválida ou menor que a local.")
                        return jsonify({'message': 'Blockchain recebida foi rejeitada.', 'received_length': len(blockchain_data), 'local_length': len(blockchain.chain)}), 400
...
def mine():
    try:
        ...
        # Calcular recompensa
        recompensa = blockchain.current_reward()

        # Atualizar saldo da carteira do minerador
        blockchain.update_balance(miner, recompensa)
        ...
        return jsonify({'error': f'Erro interno no /mine: {str(e)}'}), 500
...
if __name__ == "__main__":
    ...
    miner_address = "12Fsf93erqAzdLZ3PLYRD2A5yEmQswtadP"
